  ...
  useEffect(() => {
    const fetchCasting = async () => {
      if (!casting && castingId) {
        try {
          setLoading(true);
          const docRef = doc(db, 'castings', castingId.trim());
          const snapshot = await getDoc(docRef);
          if (snapshot.exists()) {
            const data = snapshot.data();
            setCastingData({ ...data, id: snapshot.id });
          } else {
...
const handleApplyPress = async () => {
  ...
  // Ok → Navega. Pasamos callback para incrementar el contador solo si la postulación se envía con éxito
  navigation.navigate('SubmitApplication', {
    castingId: castingData?.id,
    ...
    onSubmitted: async () => {
      ...
      try {
        ...
        console.warn('No se pudo actualizar el contador:', e?.message);
      }
    }
  });
};
...
onPress={() =>
  navigation.navigate('ViewApplications', {
    castingId: item.id,
   castingTitle: item.title || item.name || item.titulo || 'Casting',
  })
}
